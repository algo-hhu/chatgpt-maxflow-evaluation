...
std::pair<uint64_t, uint64_t> parse_problem_line(const std::string &line) {
    ...
    assert(line[0] == 'p');
    sscanf(line.c_str(), "%c %s %lu %lu", &ltype, buf, &num_nodes, &num_arcs);
...
std::pair<uint64_t, char> parse_node_line(const std::string &line) {
    ...
    uint64_t nid;
    assert(line[0] == 'n');
    sscanf(line.c_str(), "%c %lu %c", &ltype, &nid, &ntype);
    assert(ltype == 'n');
...
std::tuple<uint64_t, uint64_t, int64_t> parse_arc_line_int(const std::string &line) {
    ...
    int64_t cap;
    assert(line[0] == 'a');
    sscanf(line.c_str(), "%c %lu %lu %ld", &ltype, &from, &to, &cap);
    assert(ltype == 'a');
...
std::tuple<uint64_t, uint64_t, double> parse_arc_line_float(const std::string &line) {
    ...
    double cap;
    assert(line[0] == 'a');
    sscanf(line.c_str(), "%c %lu %lu %lf", &ltype, &from, &to, &cap);
    assert(ltype == 'a');
    return std::make_tuple(from, to, cap);
}
